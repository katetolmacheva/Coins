...
import telegram
from telegram import Bot, Update, WebAppInfo, ReplyKeyboardMarkup, KeyboardButton # Импортируем явно
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes
from sqlalchemy import create_engine, Column, Integer, String, Float
from sqlalchemy.orm import sessionmaker, declarative_base
import openpyxl
import asyncio
import platform # Для специфичных настроек asyncio на Windows
...
ADMIN_USER_ID = 123456789 # <--- ОБЯЗАТЕЛЬНО ЗАМЕНИ ЭТО НА СВОЙ ID!
...
def sync_excel_to_db():
    ...
    try:
        ...
        # Создаем маппинг заголовков Excel к атрибутам модели Coin
        column_mapping = {
            'Название': 'name',
            'Номер': 'number',
...
def get_coins():
    """API-маршрут для получения списка монет."""
        ...
        for item in items_to_reserve:
            ...
            if coin.available_quantity < quantity:
                results.append({
                    'number': coin_number, 
                    'success': False, 
                    'message': f'Недостаточно монет в наличии. Доступно: {coin.available_quantity}, Запрошено: {quantity}.'
                })
                success_all = False
                continue
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    WEBAPP_URL = "http://127.0.0.1:5000" # Или URL, который вы используете для ngrok
...
async def main():
    ...
    def run_flask():
        # ИСПРАВЛЕНО: host='0.0.0.0' для доступности извне
